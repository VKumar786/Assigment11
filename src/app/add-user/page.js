...
const AddUser = () => {
  ...
  const { accessToken, handleAddUser, successToast, errorToast } = useAuth()
  const initialValues = {
    first_name: '',
    last_name: '',
    street: '',
  ...
  const onSubmit = async (values, { setSubmitting, resetForm }) => {
    try {
      const res = await fetch('api/assignment.jsp?cmd=create', {
        method: 'POST',
        headers: {
  ...
  return (
    <Box margin={"5rem auto"} maxW={"7xl"} padding={4}>
      <Heading margin={"0 0 1rem 0"}>Customer Details</Heading>
